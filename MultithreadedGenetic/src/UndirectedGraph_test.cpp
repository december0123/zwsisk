...
protected:
    virtual void SetUp()
    {
        g_ = std::make_unique<UndirectedGraph>(5);
    }

    std::unique_ptr<UndirectedGraph> g_;
};
...
TEST_F(UndirectedGraphFixture, canBeConstructedFromFile)
{
    ...
    g_ = std::make_unique<UndirectedGraph>(filePath);
    std::stringstream ss;
    ss << *g_;
...
TEST_F(UndirectedGraphFixture, canBeConstructedAsARandomInstanceAndPrintsNicely)
{
    ...
    g_ = std::make_unique<UndirectedGraph>(NUM_OF_VERTICES, MIN_COST, MAX_COST);
    std::stringstream ss;
    ss << *g_;
    const std::string graphPrint{ss.str()};
    ss.str("");
